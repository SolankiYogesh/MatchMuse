...
import {styles} from './AwardScreenStyle'
import RenderAwardItem from './Components/RenderAwardItem'
...
const AwardScreen = () => {
  ...
  const renderPollList = () => {
    return (
      <FlatList showsHorizontalScrollIndicator={false} data={pollData} renderItem={renderItem} />
    )
  }

  const renderItem = ({item}: any) => {
    return <RenderAwardItem item={item} />
  }
    ...
    return (
      <View style={styles.headerContainer}>
        <View style={CommonStyles.row}>
          ...
          <Text style={styles.headerTitle}>{t('FD283')}</Text>
        </View>
